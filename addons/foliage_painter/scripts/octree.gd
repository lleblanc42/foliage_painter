...
#查找
func search(origin: Vector3, direction: Vector3, radial_distance: float, callback: Callable):
	_searcher.ray_search(origin,direction,radial_distance,callback)

#重新加载
func update(container:Node3D):
	if container == null:
		return
	
	for node in container.get_children():
		insert(node.position,node)
